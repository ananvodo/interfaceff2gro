...
def create_lammps_data_file(Class, frc_file, word):
    try:
        ...
        bash_command = os.path.dirname(sys.argv[0]) + '/msi2lmp/src/msi2lmp.exe ' + sys.argv[1].replace(word, '') + ' -class' + Class + '-frc ' + os.path.dirname(sys.argv[0]) + '/msi2lmp/frc_files/' + frc_file + '.frc -ignore'
...
def main():
    ...
    create_lammps_data_file(' 1 ', 'cvff_interface_v1_5', '.car');
    # create_lammps_data_file(' 2 ', 'pcff_interface', '.car');
    ...
    masselements = [1.007970, 1.008000, 15.99940, 40.08000, 22.98977, 26.98153, 26.98200, 28.08600, 30.97380, 39.09830, 107.8680, 196.9670, 63.54600, 58.71000, 207.2000, 106.4000, 195.0900]

    # converson constant between kcal and kJ
    kcal2kJ = 4.184
    ...
    for m in atoms_universe_gro:
        gro_file.write(gro_format%(1,'SURF',mass2element[int(m[1])],m[0]+1,m[2][0]/10,m[2][1]/10,m[2][2]/10))
    ...
    for m in atomtypes_universe:
        if m[1] > 10:
            itp_file.write(cformat_atomtypes%(atom_lammps_dict[int(m[0])],element2atomNumber[mass2element[int(m[1])]],m[1],0,'A',0.2*atom_ff_sigma[atom_lammps_dict[int(m[0])]],-kcal2kJ*atom_ff_epsilion[atom_lammps_dict[int(m[0])]]))
        else:
            itp_file.write(cformat_atomtypes_2%(atom_lammps_dict[int(m[0])],element2atomNumber[mass2element[int(m[1])]],m[1],0,'A',0.2*atom_ff_sigma[atom_lammps_dict[int(m[0])]],-kcal2kJ*atom_ff_epsilion[atom_lammps_dict[int(m[0])]]))
    ...
    if len(bondtypes_universe) > 0:
        ...
        for m in bondtypes_universe:
            for n in bond_ff:
                print(len(bondtypes_universe))
                print(bondtypes_universe)
                print(m)
                print(n)
